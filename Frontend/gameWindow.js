...
async function setup() {
  ...
  for (let i = 1; i < 7; i++){
    ...
    loadJSON(`../MAPS/MichaelMap1/C${i}/displaySections/c${i}ns.json`, loadNS);
    await fetch(`../MAPS/MichaelMap1/C${i}/c${i}tnames.txt`)
      .then((res) => res.text())
      .then((text) => {
        territoryNames.push(...text.split(/\r?\n/).filter(n => n));
      }).catch((e) => console.error(e));
    await fetch(`../MAPS/MichaelMap1/C${i}/c${i}nt.txt`)
      .then((res) => res.text())
      .then((text) => {
