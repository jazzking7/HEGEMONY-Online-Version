...
socket.on('war_art_tutorial', function () {
  const announ = document.getElementById('announcement');
  announ.innerHTML = `<h2>Game Setup</h2>`;
  ...
  $('#middle_content').html(`
    <div style="padding: 10px; line-height: 1.6; font-size: 1.1rem;">
      <p><strong>War Art</strong></p>
      <p>
        In this phase, you have to choose a <strong>War Art</strong> to help you.
      </p>
      <p>
        War Arts give you unique advantage, and nobody knows what War Art you have chosen.
      </p>
      <p>
        For this tutorial, you receive <strong>Mass Mobilization</strong>, a War Art that generates a lot of troops.
    </div>
  `);
...
socket.on('basic_gameplay', function () {
  const announ = document.getElementById('announcement');
  ...
  $('#middle_content').html(`
    <div style="padding: 10px; line-height: 1.6; font-size: 1.1rem;">
      <p>Now that everything is set up, players take turn do the following 4 actions:</p>
      <p>
        <strong>Reinforcement</strong> -> Add troops to your territories.
      </p>
      <p>
        <strong>Preparation</strong> -> Make preparations before attacking.
      </p>
      <p>
        <strong>Conquest</strong> -> Attack enemy territories.
      </p>
      <p>
        <strong>Rearrangement</strong> -> Rearrange your troops before your turn ends.
      </p>
    </div>
  `);
...
socket.on('reinforcement_tutorial', function () {
  const announ = document.getElementById('announcement');
  announ.innerHTML = `<h2>Basic Gameplay</h2>`;
  ...
  $('#middle_content').html(`
    <div style="padding: 10px; line-height: 1.6; font-size: 1.1rem;">
      ...
      <p>
        You can deploy up to 25 troops from your current set up. For this tutorial, deploy them all in the pointed territory to continue.
      </p>
    </div>
  `);
...
function reinforcement_tutorial(tid){
  if(clickables.includes(tid)){
    if (!toHighLight.includes(tid)){
      toHighLight.push(tid);
    }
    ...
    troopInput.setAttribute("type", "range");
    troopInput.setAttribute("min", 1);
    troopInput.setAttribute("max", 25);
    troopInput.setAttribute("value", 1);
    troopInput.setAttribute("step", 1);
    troopInput.style.display = "inline-block";
    troopInput.addEventListener("input",function(){troopValue.textContent = troopInput.value;});
    ...
    c_m.appendChild(troopValue);
    $('#control_confirm').off('click').on('click' , function(){
      if (troopInput.value == 25) {
        let c_m = document.getElementById('control_mechanism');
        c_m.innerHTML = "";
        document.getElementById('control_panel').style.display = 'none';
        socket.emit("next_tutorial_stage", { stage: 12 });
        toHighLight = [];
        clickables = [];
      }
    });
...
socket.on('preparation_tutorial', function () {
  const announ = document.getElementById('announcement');
  announ.innerHTML = `<h2>Basic Gameplay</h2>`;
  ...
  $('#middle_content').html(`
    <div style="padding: 10px; line-height: 1.6; font-size: 1.1rem;">
      ...
      <p>
        In this stage, you can prepare for war, there are 4 buttons on the bottom-left corner that can help you with that.
      </p>
      <p>Starting from left to right, there are:</p>
      <p>
        <strong>Diplomatic Actions Button</strong> -> Able to launch timeout between turns or Global Ceasefire.
      </p>
      <p>
        <strong>Special Authority Button</strong> -> Spend Stars to build upgrades.
      </p>
      <p>
        <strong>War Art Button</strong> -> Use your War Art.
      </p>
      <p>
        <strong>Reserve Button</strong> -> Deploy Reserves (hidden extra troops that only you can see and use).
      </p>
    </div>
  `);
  ...
  $('#proceed_next_stage')
    ...
    .on('click', () => {
      $('#proceed_next_stage').hide();
      socket.emit("next_tutorial_stage", {'stage': 13});
    });
...
function conquest_tutorial(tid) {
  if (!clickables.includes(tid)) return;

  if (tid === 7 && !toHighLight.includes(7)) {
    // Reset clickable territories, only allow 25 next
    clickables = [];
    clickables.push(25);

    // Highlight 7
    if (!toHighLight.includes(7)) {
      toHighLight.push(7);
    }

  } else if (tid === 25 && toHighLight.includes(7)) {
    // Highlight 25
    if (!toHighLight.includes(25)) {
      toHighLight.push(25);
    }
...
function rearrangement_tutorial(tid) {
  if (!clickables.includes(tid)) return;

  if (tid === 25 && !toHighLight.includes(25)) {
    // Reset clickable territories, only allow 25 next
    clickables = [];
    clickables.push(7);
  ...
  } else if (tid === 7 && toHighLight.includes(25)) {
    // Highlight 25
    if (!toHighLight.includes(7)) {
      toHighLight.push(7);
    }
...
function teardownTutorial() {
  // 1) Clear timeouts/intervals & progress bars
  if (current_interval) { clearInterval(current_interval); current_interval = null; }
  try { if (timeoutBar && timeoutBar.stop) timeoutBar.stop(); } catch(e) {}

  // 2) Unbind DOM listeners we attached
  $(document).off('click');                               // your global doc click
  $('#overlay_sections .card').off('click mousemove');
  $('#overlay_sections .circular-button').off('click mousemove');

  // 3) Remove or reset global state used by the sketch
  window.currEvent = null;
  if (window.clickables) window.clickables = [];
  if (window.toHighLight) window.toHighLight = [];
  ...
  [
    ...
    'rearrangement_tutorial','end_of_tutorial',
    'change_click_event_tutorial','initiate_tracker_for_tutorial','update_tracker',
    'start_timeout','stop_timeout','get_players_stats','update_players_stats',
    'update_global_status','private_overview','update_player_territories',
    'mod_targets_to_capture','update_trty_display'
  ].forEach(evt => socket.off(evt));

  // 5) If a p5 sketch/canvas is running, remove it to stop mouse handlers
  // (use your own instance ref if you have one; fallback removes canvases in the game container)
  try {
...
socket.on('change_click_event_tutorial', function(data){
  ...
  } else if (data.event == 'reinforcement_tutorial'){
    currEvent = reinforcement_tutorial;
  } else if (data.event == 'conquest_tutorial'){
    currEvent = conquest_tutorial;
  } else if (data.event == 'rearrangement_tutorial'){
    currEvent = rearrangement_tutorial;
